...
export default class JsonRpc extends Rpc
{
  ...
  async onMessage(message)
  {
    ...
    // Decode
    const {error, id: ack, jsonrpc: data, method, ...rest} = message
    ...
    if(id === null)
    {
      ...
      console.warn('Using of `null` as requests `id` is discouraged')
    }

    return super.onMessage({[method ? 'id' : 'ack']: ack, error, method, ...rest})
  }
