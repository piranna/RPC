...
export default class JsonRpc extends Rpc
{
  ...
  async onMessage(data)
  {
    ...
    {
      const error = {code: -32700, data, message: 'Parse error'}
    ...
    {
      const error = {code: -32600, message: 'Empty batch request'}
    ...
    if(error)
    {
      ...
      if(isError(error))
      {
        ...
        if(this.#sendFullErrors)
          result.error = errToJSON(error)
        else
        {
          ({uuid: data} = error)

          // Remove stack trace
          result.error = {code, data, message}
        }
      }
